...
foreach ($modules as $const => $desc) {
	// When the max depth was never submitted, we use the same default as the one shown in the max depth
	// input below and as the one used by CommonSubtotal::getPossibleLevels(), otherwise no color could be
	// set until the max depth is saved.
	$module_enabled = getDolGlobalInt('SUBTOTAL_TITLE_' . $const) || getDolGlobalInt('SUBTOTAL_' . $const);
	$const_depth = getDolGlobalInt('SUBTOTAL_' . $const . '_MAX_DEPTH', $module_enabled ? 2 : 0);
