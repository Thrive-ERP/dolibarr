	...
	public function addline($entrepot_id, $id, $qty, $array_options = [], $comment = '', $eatby = null, $sellby = null, $batch = '', $cost_price = 0)
	{
		...
		if (isModEnabled('productbatch')) {
			...
			if (!empty($product->status_batch) && empty($batch)) {
				//Begin Customisation: Accellier Ltd: AUto Gen the Serial and Lot Number
				// Auto-generate the batch/serial number when the field was left blank.
				// getNextValue() cannot see lines added earlier in the same open transaction,
				// so we walk $this->lines in memory to derive the correct next counter.

				// Each serial number represents one physical unit; reject qty > 1 for SN products.
				if ($product->status_batch == 2 && $qty != 1) {
					$this->error = 'Product '.$product->ref.' is serial-number controlled: each reception line must have qty 1 (got '.(float)$qty.').';
					return -1;
				}
				...
				if ($product->status_batch == 2) {
					if ((getDolGlobalString('PRODUCTBATCH_SN_ADDON') == 'mod_sn_advanced') || (getDolGlobalString('PRODUCTBATCH_SN_ADDON') == 'mod_sn_standard')) {
						...
						dol_include_once($batch_autogen_dirroot.$batch_autogen_module.'.php');
						...
						$batch_autogen_object->fk_product = $fk_product; // enables product-level mask in getNextValue()
						$batch = $batch_autogen_mod->getNextValue(null, $batch_autogen_object);

						// Advance past serials already queued in this transaction but not yet in DB.
						// Use strict !== '' because PHP evaluates ('' >= 0) as true, which would
						// incorrectly match the "no prior line" initial value.
						if (!empty($this->lines)) {
							$lastKey = '';
							...
							if ($lastKey !== '') {
								$lastbatch = $this->lines[$lastKey]->batch;
								if (getDolGlobalString('PRODUCTBATCH_SN_ADDON') == 'mod_sn_advanced') {
									$mask = (getDolGlobalString('PRODUCTBATCH_SN_USE_PRODUCT_MASKS') && !empty($product->batch_mask))
										? $product->batch_mask
										: getDolGlobalString('SN_ADVANCED_MASK');
									if (preg_match('/\{(0+)([@\+][0-9\-\+\=]+)?([@\+][0-9\-\+\=]+)?\}/i', $mask, $reg)) {
										...
										if (($batch_pre != $lastbatch) && !empty($batch_counter)) {
											...
											$batch = $batch_pre.sprintf("%0".$maskcounter_len."d", ((int) $batch_counter + $step)).$batch_suf;
										}
									}
								} elseif (getDolGlobalString('PRODUCTBATCH_SN_ADDON') == 'mod_sn_standard') {
									// "PREFIX-NNNN" format
									$batch_array = explode('-', $lastbatch);
									$batch_array[1] = (int) $batch_array[1] + 1;
					...
					if ((getDolGlobalString('PRODUCTBATCH_LOT_ADDON') == 'mod_lot_advanced') || (getDolGlobalString('PRODUCTBATCH_LOT_ADDON') == 'mod_lot_standard')) {
						...
						// Advance past lots already queued in this transaction but not yet in DB.
						if (!empty($this->lines)) {
							$lastKey = '';
							foreach ($this->lines as $key => $linevalue) {
								$last_product = new Product($this->db);
							...
							if ($lastKey !== '') {
								$lastbatch = $this->lines[$lastKey]->batch; // increment from last queued lot, not the new $batch
								if (getDolGlobalString('PRODUCTBATCH_LOT_ADDON') == 'mod_lot_advanced') {
									$mask = (getDolGlobalString('PRODUCTBATCH_LOT_USE_PRODUCT_MASKS') && !empty($product->batch_mask))
										? $product->batch_mask
										: getDolGlobalString('LOT_ADVANCED_MASK');
									if (preg_match('/\{(0+)([@\+][0-9\-\+\=]+)?([@\+][0-9\-\+\=]+)?\}/i', $mask, $reg)) {
										$maskcounter = $reg[1];
										$maskcounter_start = strpos(str_replace("}", "", str_replace("{", "", $mask)), $maskcounter);
										$maskcounter_len = strlen($maskcounter);
